...
@section('breadcrumb',__('Hall slider')  )
...
@section('content')
   <div class="card">
                ...
                <div class="col-lg-6 col-md-6">
                    <div class="row">
                        <div class="col-lg-8 col-md-8">
                            <form action="{{ route('get_hall') }}" method="POST" id="hall_change">
                                @csrf
                                <select class="form-control" name="hall_id" id="halls">
                                    <option value="">{{ __('Choose') }}</option>
                                    @foreach ($halls as $item)
                                        <option @if (session('hall_id') == $item->id) {{ 'selected' }} @endif value="{{ $item->id }}">
    ...
    <script>
        ...
        function call() {
            var url = "{{ route('get_hall') }}";
            $.ajax({
                type: "post",
                url: url,
                data: $("#hall_change").serialize(),
                cache: false,
                beforeSend: function() {
                    $("#hall_content").html(
            ...
            $.ajax({
                ...
                success: function(data) {
                   // console.log(data);
                    ...
                    $("#hall_id option[value='" + data['0']['hall_id'] + "']").attr("selected", "selected");
                }
            })
        }
    ...
    @include('Back.InnerHalls.edit')
    @include('Back.InnerHalls.add')
@endsection
